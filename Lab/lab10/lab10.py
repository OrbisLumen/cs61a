...
def floor_div(args):
    ...
    while temp is not nil:
        if isinstance(temp.first, int):
            ans //= temp.first
        elif isinstance(temp.first, Pair):
            ans //= calc_eval(temp.first)
        temp = temp.rest
    return ans
...
def eval_and(expressions):
    ...
    last = t.first
    while t is not nil:
        if isinstance(t.first, Pair):
            last = calc_eval(t.first)
        else:
            last = t.first
        if last is scheme_f:
            return scheme_f
...
def eval_define(expressions):
    ...
    if exp.first in bindings:
        bindings[key] = bindings[exp.first]
    elif isinstance(exp.first, Pair):
        bindings[key] = calc_eval(exp.first)
    elif exp.first in OPERATORS:
        bindings[key] = OPERATORS[exp.first]
    else:
        bindings[key] = exp.first
    return key

...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        # distance = 0
        # while True:
        #     if finder.entrance.is_hive:
        #         if distance >= self.lower_bound and distance <= self.upper_bound:
        #             return random_bee(finder.bees)
        #         else:
        #             return None
        #     elif finder.bees == []:
        #         finder = finder.entrance
        #         distance += 1
        #         continue
        ...
        finder = self.place
        distance = 0
        while not finder.is_hive:
            if finder.bees and self.lower_bound <= distance <= self.upper_bound:
                return random_bee(finder.bees)
            finder = finder.entrance
            distance += 1
        return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= 0:
            hit += self.damage
        for b in list(place.bees[:]):
            Insect.reduce_health(b, hit)
        # END Problem 5
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing each turn, but has a large health value"""
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt eats one of the bees in its place"""
    ...
    def action(self, gamestate):
        if self.cooldown == 0 and self.place.bees:
            ...
            Insect.reduce_health(target, target.health)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        super().action(gamestate)
        double_target = self.place.exit
        while double_target:
            if not double_target.ant:
                double_target = double_target.exit
                continue
            if double_target.ant.is_container:
                if not double_target.ant.is_doubled:
                    double_target.ant.is_doubled = True
                    double_target.ant.damage *= 2
                if (
                    double_target.ant.ant_contained
                    and not double_target.ant.ant_contained.is_doubled
                ):
                    double_target.ant.ant_contained.is_doubled = True
                    double_target.ant.ant_contained.damage *= 2
            else:
                if not double_target.ant.is_doubled:
                    double_target.ant.is_doubled = True
                    double_target.ant.damage *= 2

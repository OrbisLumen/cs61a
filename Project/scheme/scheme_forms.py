...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        sign = expressions.first.first
        formals = expressions.first.rest
        procedure = expressions.rest
        exp = Pair(sign, Pair(Pair("lambda", Pair(formals, procedure)), nil))
        return do_define_form(exp, env)
        # END PROBLEM 10
    else:
...
def do_lambda_form(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    procedure = expressions.rest
    return LambdaProcedure(formals, procedure, env)
    # END PROBLEM 7
...
def do_and_form(expressions, env):
    ...
    while target is not nil:
        ans = scheme_eval(target.first, env)
...
def do_mu_form(expressions, env):
    ...
    formals = expressions.first
    procedure = expressions.rest
    return MuProcedure(formals, procedure)
    # END PROBLEM 11
